...
interface TabKenaikanProps {
  siswaList: SiswaPelengkapItem[];
  tingkat: number;
  isKelasAkhir: boolean;
  isSemesterGenap: boolean;
  loadingId: string | null;
  isBulkSaving: boolean;
  onStatusKenaikanChange: (siswaId: string, val: string) => void;
  onBulkSetKenaikan: (val: string) => void;
  onBulkSaveKenaikan: () => Promise<void>;
  onSaveIndividual: (siswa: SiswaPelengkapItem) => Promise<void>;
  onRevertIndividual: (siswaId: string) => void;
  onRevertAll: () => void;
}

export function TabKenaikan({
  siswaList,
  tingkat,
  isKelasAkhir,
  isSemesterGenap,
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasUnsavedChanges && (
              <button
                ...
                title="Batalkan seluruh perubahan status kenaikan yang belum disimpan"
              >
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-amber-200/60">
          ...
          {isKelasAkhir ? (
            <>
              <button
                type="button"
                onClick={() => onBulkSetKenaikan("LULUS")}
              ...
              <button
                type="button"
                onClick={() => onBulkSetKenaikan("TIDAK LULUS")}
            ...
            <>
              <button
                type="button"
                onClick={() => onBulkSetKenaikan(`Naik ke Kelas ${tingkat + 1}`)}
                className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1 rounded-lg font-semibold transition"
              >
              ...
              <button
                type="button"
                onClick={() => onBulkSetKenaikan(`Tinggal di Kelas ${tingkat}`)}
                className="text-xs bg-rose-600 hover:bg-rose-700 text-white px-3 py-1 rounded-lg font-semibold transition"
              >
        ...
        <table className="w-full text-xs text-left">
          <thead className="bg-stone-100 text-zinc-700 font-bold border-b border-stone-200 uppercase tracking-wider text-[11px]">
            <tr>
              ...
              <th className="py-3 px-4 w-36 text-center">NISN</th>
              <th className="py-3 px-4 w-40 text-center">Status Simpan</th>
                ...
                <tr key={s.id} className="hover:bg-stone-50/60 transition">
                  ...
                  <td className="py-3 px-4">
                    <select
                      ...
                      onChange={(e) => onStatusKenaikanChange(s.id, e.target.value)}
                    ...
                    <div className="flex items-center justify-center gap-1.5">
                      {!s.isKenaikanSaved && (
                        <button
                          ...
                          title="Batalkan perubahan untuk siswa ini"
                        >
                          <RotateCcwIcon className="h-3.5 w-3.5" />
                          <span>Batal</span>
                        </button>
